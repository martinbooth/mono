...
gchar **do_not_finalize_class_names = NULL;
...
mono_gc_run_finalize (void *obj, void *data)
{
	...
	if (do_not_finalize) {
		if (!do_not_finalize_class_names)
			return;

		size_t namespace_len = strlen (o->vtable->klass->name_space);
		for (int i = 0; do_not_finalize_class_names [i]; ++i) {
			const char *name = do_not_finalize_class_names [i];
